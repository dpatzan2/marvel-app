...
const HeroModal = ({ character, isOpen, onClose }: HeroModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      <div 
        className={`absolute inset-0 bg-black/90 ${!isMobile ? 'backdrop-blur-strong' : ''}`}
        onClick={onClose}
      >
      ...
      <Card className={`relative w-full max-w-5xl max-h-[95vh] overflow-hidden bg-card/90 border-primary/40 animate-slide-in ${
        !isMobile ? 'backdrop-blur-strong card-glow hologram' : 'bg-card'
      }`}>
        ...
        <Button
          ...
          size="sm"
          onClick={onClose}
          className={`absolute top-3 right-3 sm:top-6 sm:right-6 z-20 text-white hover:text-accent hover:bg-accent/20 rounded-full w-8 h-8 sm:w-10 sm:h-10 transition-all duration-300 ${
        ...
        <div className={`absolute top-0 left-0 right-0 h-2 ${
          isMobile ? 'bg-primary/50' : 'hero-gradient'
        }`}></div>
        ...
        <CardContent className="p-0 relative overflow-hidden">
          <div className="flex flex-col lg:grid lg:grid-cols-2 gap-0 max-h-[95vh] overflow-y-auto scrollbar-hidden">
              ...
              {isValidImage ? (
                <>
                  <img
                    src={imageUrl}
                    alt={character.name}
                    className={`w-full h-full min-h-[250px] sm:min-h-[300px] lg:min-h-[600px] object-cover transition-all duration-300 gpu-accelerated ${
                  ...
                  <div className={`absolute inset-0 ${
                    isMobile 
                      ? 'bg-gradient-to-r from-primary/30 to-accent/30 opacity-60'
                      : 'image-overlay opacity-60'
                  }`}></div>
                ...
                <div className="w-full min-h-[250px] sm:min-h-[300px] lg:min-h-[600px] h-full bg-gradient-to-br from-primary/30 via-accent/20 to-secondary/30 flex items-center justify-center relative overflow-hidden">
                  {!isMobile && <div className="cyber-grid absolute inset-0 opacity-30"></div>}
                  <div className="text-center z-10">
                    <Shield className={`w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 text-primary mb-4 mx-auto ${
                      !isMobile ? 'animate-pulse-glow' : ''
                    }`} />
                    <span className={`text-3xl sm:text-4xl lg:text-6xl font-orbitron font-bold text-primary/70 ${
                      !isMobile ? 'text-glow' : ''
                    }`}>
                      {character.name.charAt(0)}
                    </span>
                  </div>
                  {!isMobile && <div className="absolute inset-0 image-overlay opacity-40"></div>}
                </div>
              )}
            ...
            <div className={`p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8 relative overflow-y-auto scrollbar-hidden ${
              !isMobile ? 'glass-morphism' : 'bg-card/80'
            }`}>
              {/* Character header */}
              <div className="space-y-2 sm:space-y-4">
                <div className="flex items-center space-x-2 sm:space-x-3 mb-2 sm:mb-4">
                  <Zap className={`w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-primary flex-shrink-0 ${
                    !isMobile ? 'animate-pulse' : ''
                  }`} />
                  <div className="min-w-0">
                    <h2 className={`text-2xl sm:text-3xl lg:text-4xl font-orbitron font-bold text-primary leading-tight ${
                      !isMobile ? 'text-glow' : ''
                    }`}>
                      {character.name}
                    </h2>
                    <Badge className={`bg-primary/20 text-primary border-primary/50 mt-1 sm:mt-2 text-xs ${
                      !isMobile ? 'glow-blue' : ''
                    }`}>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:gap-4">
                  <div className={`flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 lg:p-4 bg-gradient-to-r from-primary/10 to-primary/5 rounded-lg border border-primary/30 hover:border-primary/50 transition-all duration-300 ${
                    !isMobile ? 'glow-blue' : ''
                  }`}>
                  ...
                  <div className={`flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 lg:p-4 bg-gradient-to-r from-accent/10 to-accent/5 rounded-lg border border-accent/30 hover:border-accent/50 transition-all duration-300 ${
                    !isMobile ? 'glow-red' : ''
                  }`}>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  <Button
                    ...
                    className={`border-primary/40 hover:border-primary hover:bg-primary/10 transition-all duration-300 group text-xs sm:text-sm ${
                      !isMobile ? 'glow-blue' : ''
                    }`}
                  >
                    <ExternalLink className={`w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 transition-transform ${
                      !isMobile ? 'group-hover:scale-110' : ''
                    }`} />
                    Marvel DB
                  </Button>
                  ...
                  <Button
                    ...
                    className={`border-accent/40 hover:border-accent hover:bg-accent/10 transition-all duration-300 group text-xs sm:text-sm ${
                      !isMobile ? 'glow-red' : ''
                    }`}
                  >
                    <Book className={`w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 transition-transform ${
                      !isMobile ? 'group-hover:scale-110' : ''
                    }`} />
                    Cómics
                  </Button>
                  ...
                  >
                    <Shield className={`w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 transition-transform ${
                      !isMobile ? 'group-hover:scale-110' : ''
                    }`} />
                    Wiki
                  </Button>
